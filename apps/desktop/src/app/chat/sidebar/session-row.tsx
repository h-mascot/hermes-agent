...
function SidebarSessionRowImpl({
  ...
  if (figures.length) {
    ...
    // match; the one-line row keeps the interpunct between joined figures.
    const sep = card ? '  ' : ' · '
  ...
  // Card footer line: which model worked on it and how big it got.
  ...
  const meta = [model, size].filter(Boolean).join('  ')
